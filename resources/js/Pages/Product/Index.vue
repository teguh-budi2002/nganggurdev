<template>
  ...
  <div class="lg:mt-20 mt-2 h-full min-h-screen w-full dark:bg-slate-900">
    <div class="flex flex-col justify-center items-center">
      <div class="lg:w-10/12 w-11/12 h-full mt-20">
        ...
        <div class="lg:w-3/4 w-full mx-auto lg:mt-0 mt-4">
          <p class="lg:text-center text-start font-['Roboto'] lg:text-5xl text-3xl capitalize font-semibold lg:leading-15 leading-11 dark:text-slate-100">{{ $t('hero_text.products.1') }} 
            <span class="bg-gradient-to-r from-rose-600 to-rose-300 dark:from-slate-700 dark:to-slate-500 text-white p-1 px-2">{{ $t('delicious_snack') }}</span> {{ $t('hero_text.products.2') }}
          </p>
        </div>
              ...
              <div class="mt-5">
                <div class="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-2" v-if="snackProducts.length > 0">
                  <template v-for="product in snackProducts" :key="product.id">
                    <div class="w-full shadow-sm rounded-md hover:shadow-lg transition-shadow duration-200 dark:bg-slate-700">
                      ...
                      <div class="p-4">
                        ...
                        <a :href="product.anchor_link" class="w-full font-semibold font-['Inter'] hover:text-sky-600 dark:text-slate-100 dark:hover:text-slate-300 transition-colors duration-200" target="_blank">{{ truncate(product.title, 59) }}</a>
                        <div class="mt-2 space-y-4">
                          ...
                          <div class="flex items-center justify-between">
                            <p class="font-['Inter'] font-semibold text-green-600 dark:text-green-500">{{ formatCurrencyIDR(product.price_after_discount) }}</p>
                            <p class="font-['Inter'] line-through text-rose-600 dark:text-rose-500 text-xs font-light">{{ formatCurrencyIDR(product.price) }}</p>
                          </div>
                          <div class="border-t border-slate-300 dark:border-slate-400 pt-2 flex items-center space-x-1">
                            <img src="/assets/images/icon/star.svg" alt="star icon" class="size-5">
                            <p class="font-['Inter'] text-sm font-semibold text-slate-500 dark:text-slate-300">5.0</p>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div v-else class="text-center mt-10">
                  <p class="md:text-xl text-sm font-['Inter'] text-slate-500 dark:text-slate-300">No products available in this category.</p>
                </div>
              </div>
            </template>
                ...
                <div class="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-2" v-if="applicationProducts.length > 0">
                  <template v-for="product in applicationProducts" :key="product.id">
                    <div class="w-full shadow-sm rounded-md hover:shadow-lg transition-shadow duration-200 dark:bg-slate-700">
                      ...
                      <div class="p-4">
                        <a href="" target="_blank" class="w-full font-semibold font-['Inter'] hover:text-sky-600 dark:text-slate-100 dark:hover:text-slate-300 transition-colors duration-200">{{ truncate(product.title, 59) }}</a>
                        <div class="mt-2 space-y-4">
                          <button type="button" class="p-1 animate-pulse rounded-md text-white bg-rose-600 text-xs font-bold uppercase">Diskon {{ product.discount }}%</button>
                          <div class="flex items-center justify-between">
                            <p class="font-['Inter'] font-semibold text-green-600 dark:text-green-500">{{ formatCurrencyIDR(product.price_after_discount) }}</p>

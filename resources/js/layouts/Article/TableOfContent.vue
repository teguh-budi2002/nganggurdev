<template>
<div class="w-full h-fit shadow rounded-md bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-600 p-6">
  <div class="header_table_of_content flex items-center space-x-2">
    ...
    <p class="font-['Roboto'] font-semibold text-slate-600 dark:text-slate-100">Table of Content</p>
  </div>
      ...
      <template v-for="item in toc" :key="item.id">
        <div class="flex items-center space-x-3" v-if="item.level !== 'p'">
          ...
          <a 
            ...
            class="text-sm leading-snug font-['Roboto'] transition-colors duration-150"
            :class="{
              'text-rose-600 hover:text-rose-600 dark:hover:text-slate-300 dark:text-slate-300 font-semibold': activeSection === item.id,
              'dark:text-slate-400 dark:hover:text-slate-300' : activeSection !== item.id
            }"
          >
            {{ item.text }}
          </a>

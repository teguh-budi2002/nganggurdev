  ...
  <div class="landing_page lg:mt-20 h-full w-full">
    <div class="grid lg:grid-cols-2 grid-cols-1 lg:mt-20 mt-10 bg-rose-100">
      <div class="hero_text_section p-8 flex flex-col justify-center space-y-6">
        <p class="md:text-7xl text-4xl font-['Open_Sans'] font-extrabold text-transparent bg-clip-text bg-gradient-to-l from-yellow-600 via-orange-500 to-rose-500 leading-[1.2] overflow-visible">{{ $t('brand_name') }}</p>
        <div class="md:mt-4">
          <p class="md:text-5xl text-3xl font-['Open_Sans'] italic font-light md:leading-13 text-slate-600">
            "{{ $t('hero_text.part1') }} <span class="font-bold">{{ $t('hero_text.part2') }}</span>"
          </p>
          <p class="font-['Roboto'] md:text-base text-sm capitalize font-light text-slate-700 mt-3">
            {{ $t('sub_text.1') }} 
            <span class="text-red-700">| {{ $t('sub_text.2') }}</span>
          </p>
        </div>
        <div class="mt-2">
          <Link :href="route('articles.index', { locale })" class="py-2 px-8 bg-gradient-to-r shadow-lg from-rose-500 from-45% to-rose-300 hover:from-rose-700 hover:to-rose-500 transition-colors duration-500 text-white rounded-md font-['Open_Sans'] text-sm">{{ $t('button.landing_page.text') }}</Link>
        </div>
      </div>
      ...
      <div class="flex flex-col items-center justify-center">
        ...
        <div class="md:w-2/3 w-11/12 mt-5">
          <Vue3Marquee 
            ...
            :duration="35"
            :direction="'normal'">
            <div v-for="image in imageIcons2" :key="image" class="md:w-60 w-40 flex justify-center items-center space-x-4 md:p-4 p-2 mx-4 border border-gray-200 rounded-md">

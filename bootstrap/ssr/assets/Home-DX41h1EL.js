import { ref, mergeProps, unref, withCtx, createVNode, createTextVNode, toDisplayString, useSSRContext, onMounted, createBlock, openBlock, Fragment, renderList } from "vue";
...
const _sfc_main$3 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "w-full h-full mt-10" }, _attrs))}><div class="flex items-center justify-center"><div class="w-11/12 h-full"><div><p class="text-center md:text-5xl text-3xl font-[&#39;Bebas_Neue&#39;] font-bold text-slate-600 tracking-wider">${_ctx.$t("article.new") ?? ""}</p></div><p class="text-center font-[&#39;Roboto&#39;] md:text-base text-sm font-light">${ssrInterpolate(_ctx.$t("collection.material"))}</p>`);
      if (articles.value.length > 0) {
        _push(`<div class="articles grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4 md:mt-10 mt-8 mb-10"><!--[-->`);
        ssrRenderList(articles.value, (article) => {
          _push(`<div class="w-full h-fit hover:shadow-xl transition-shadow duration-300 shadow-sm">`);
          _push(ssrRenderComponent(unref(Link), {
            href: _ctx.route("article.show", { locale: unref(locale), slug: article.slug })
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<img${ssrRenderAttr("src", `/storage/${article.image}`)} alt="article image" class="w-full max-h-56 cursor-pointer rounded-t-lg"${_scopeId}>`);
              } else {
                return [
                  createVNode("img", {
                    src: `/storage/${article.image}`,
                    alt: "article image",
                    class: "w-full max-h-56 cursor-pointer rounded-t-lg"
                  }, null, 8, ["src"])
                ];
              }
            }),
          ...
          _push(`<div class="px-4 py-4 h-56 flex flex-col justify-between border-l border-r border-b border-slate-100 rounded-b-lg"><div>`);
          _push(ssrRenderComponent(unref(Link), {
            href: _ctx.route("article.show", { locale: unref(locale), slug: article.slug }),
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(unref(truncate)(article.title, 158))}`);
              } else {
                return [
                  createTextVNode(toDisplayString(unref(truncate)(article.title, 158)), 1)
                ];
              }
            }),
            _: 2
          }, _parent));
          _push(`</div><div class="authror w-full flex items-center justify-between border-t border-slate-300 pt-2"><div class="categories flex items-center space-x-2"><!--[-->`);
          ssrRenderList(article.categories, (category) => {
            _push(`<div class="w-fit">`);
            _push(ssrRenderComponent(unref(Link), {
              href: "",
              class: ""
            }, {
              default: withCtx((_, _push2, _parent2, _scopeId) => {
                if (_push2) {
                  _push2(`<img${ssrRenderAttr("src", `/storage/${category.img_category}`)} class="w-5 h-5" alt="logo icon"${_scopeId}>`);
                } else {
                  return [
                    createVNode("img", {
          ...
          _push(`<!--]--></div><p class="font-[&#39;Roboto&#39;] text-sm font-light text-slate-700 text-end">${ssrInterpolate(article.author.name)}</p></div></div></div>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<div class="flex justify-center w-full"><img src="/assets/images/article-not-found.webp" alt="article not found" class="w-96 mx-auto h-auto"></div>`);
      }
      _push(`</div></div><div class="flex justify-center"><div class="bg-white w-fit mt-5 mb-10 p-2 px-4 border border-slate-200 rounded-full shadow-sm animate-bounce"><p class="font-[&#39;Roboto&#39;] text-slate-700 flex items-center space-x-1">${ssrInterpolate(_ctx.$t("visit.more.articles"))}? `);
      _push(ssrRenderComponent(unref(Link), {
...
const _sfc_main = /* @__PURE__ */ Object.assign({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div><div class="hero_image_section lg:block hidden"><img src="/assets/images/hero-image.png" alt="Hero Image" class="w-2/3 h-auto mx-auto"></div></div></div><section><div class="w-full h-full"><div class="flex flex-col items-center justify-center"><div class="md:w-2/4 w-3/4 mt-10">`);
      ...
      _push(ssrRenderComponent(_sfc_main$3, { articles: articles.value }, null, _parent));
